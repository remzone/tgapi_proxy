...
При открытии домена в обычном браузере отображается нейтральная страница-заглушка без сведений о назначении сервиса. API обрабатывает только пути Telegram Bot API вида `/bot...` и `/file/bot...`.
